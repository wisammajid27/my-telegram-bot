...
def parse_birth_date(date_str: str) -> datetime:
    """تحليل تاريخ الميلاد مع دعم الفاصل - أو / أو ."""
...
def normalize_birth_date(date_str: str) -> str:
    """توحيد التاريخ بصيغة dd-mm-yyyy للتخزين والحساب."""
...
        context.user_data['step'] = "add_member"
        await query.message.edit_text("👤 أدخل اسم الشخص + تاريخ الميلاد:\nمثال: `أحمد 15-05-1995` أو `أحمد 15/05/1995` أو `أحمد 15.05.1995`")
...
    if step == "quick_calc":
        try:
            # التحقق من صحة تاريخ الميلاد المدخل (- أو / أو .)
